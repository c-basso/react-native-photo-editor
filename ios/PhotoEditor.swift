...
class PhotoEditor: NSObject, ZLEditImageControllerDelegate {
    ...
    private func setConfiguration(options: NSDictionary, resolve:@escaping RCTPromiseResolveBlock,reject:@escaping RCTPromiseRejectBlock) -> Void{
        ...
        let tools = options["tools"] as? [String] ?? []
        
        var editImageTools: [ZLImageEditorConfiguration.EditImageTool] = [.draw, .clip, .filter, .imageSticker, .textSticker];
    ...
    private func getUIImage (
        ...
        if let path = URL(string: url), path.host != nil {
            ...
            if FileManager.default.fileExists(atPath: url) {
                DispatchQueue.main.async {
                    if let image = UIImage.init(contentsOfFile: url) {
                        completion(image)
                    }
                }
            } else {
